...
let app;
let newElement; // Timeout used for demo purposes

setTimeout(e => {
  app = new _Modules_Elements__WEBPACK_IMPORTED_MODULE_0__["Elements"]();
  newElement = app.create('.first', aniObject);
  demo();
}, 2601); // letters

const spans = document.querySelectorAll('.title--animated span'); // buttons
...
const tZ = document.getElementById('translateZ'); // phone message
...
function demo() {
  ...
  rX.addEventListener('click', e => {
    newElement.transforms['rotateX'].active = newElement.transforms['rotateX'].active ? false : true;
  });
  rY.addEventListener('click', e => {
    newElement.transforms['rotateY'].active = newElement.transforms['rotateY'].active ? false : true;
  });
  rZ.addEventListener('click', e => {
    newElement.transforms['rotateZ'].active = newElement.transforms['rotateZ'].active ? false : true;
  });
  tX.addEventListener('click', e => {
    newElement.transforms['translateX'].active = newElement.transforms['translateX'].active ? false : true;
  });
  tY.addEventListener('click', e => {
    newElement.transforms['translateY'].active = newElement.transforms['translateY'].active ? false : true;
  });
  tZ.addEventListener('click', e => {
    newElement.transforms['translateZ'].active = newElement.transforms['translateZ'].active ? false : true;
  });
  rX.click();
  rY.click();
}
